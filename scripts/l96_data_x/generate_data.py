...
from l96 import generate_l96_data
...
if generate_training_data:
    if not os.path.exists(f"{data_folder}/training_params.pth"):
        GT_min, GT_max = (
            np.array(args.sample_prior_min),
            np.array(args.sample_prior_max),
        )
        GT_params = np.round(
            np.random.uniform(low=0, high=1, size=(args.num_of_sample, 1)), 4
        )
    ...
    for i in tqdm(range(0, int(args.num_of_sample / num))):
        ...
        for j in range(0, split):
            ...
            params = lorenz_params_train[
                int(i * num + num / split * j) : int(i * num + num / split * (j + 1))
            ]
            params_cat_seed = np.concatenate(
                [
                    params,
                    (
                        random_index
                        + np.arange(params.shape[0])
                        + int(i * num + j * num / split)
                    )[:, None],
                ],
                axis=-1,
            )
            ...
            for ix in range(params.shape[0]):
                torch.save(
                    {"0": params[ix], "1": total_traj[ix]},
                    "{}/{:06d}.pth".format(
                        data_folder, int(i * num + ix + num / split * j)
                    ),
                )
