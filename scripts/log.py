...
def create_folder_path(args):
    ...
    if args.with_geomloss > 0:
        ...
        if args.with_geomloss_kd > 0:
            print(
                "We are using OT loss *without* the perfect knowledge of the dynamics."
            )
            print(
                "The detailed configuration is controlled by the command value of args.with_geomloss_kd"
            )
