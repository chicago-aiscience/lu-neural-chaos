...
def load_model(saved_pth, operator, optimizer):
    ...
    if len(saved_pth_list) > 0:
        ...
        if epoch_id.max() > 0:
            ...
            load_pth = "{saved_pth}_{}".format(
                visual_folder, prefix, args.special_prefix, epoch_load
            )
            ...
            checkpoint_optimizer = {
                key.replace("module.", ""): val
                for key, val in checkpoint["optimizer_state_dict"].items()
            }
            try:
...
def long_length_predict_with_yinit(operator, y, param, x_len, len_to_operator):
    ...
    for ix in range(len_to_operator - 1):
        params_ = (
            param[:, None, 0]
            .repeat(1, len(initial_index))
            .reshape(batch_size * len(initial_index), -1)
        )
    ...
    prediction_out = (
        torch.stack(prediction_list, axis=2).squeeze().reshape(batch_size, -1, x_res)
    )
    if match_length_flag:
        prediction_out = torch.cat(
            [
                prediction_out.reshape(batch_size, -1, x_res),
                y[:, x_len - 1, :][:, None, :],
            ],
            dim=1,
        )
...
def visualiztion(dataset, operator, args, img_pth, ep):
    ...
    for ix_data in range(3):
        ...
        plot_3col(
            ...
            [out, "predict"],
            operator="-",
            im=f"{img_pth}/{ep:03d}_{ix_data}",
        )


def adjust_tau_metricL(
    T_metricL_ori, train_epoch, total_epochs, epochs=100, max_tau_metricL=1
):
    if train_epoch <= (total_epochs - epochs):
        T_metricL = T_metricL_ori + (max_tau_metricL - T_metricL_ori) * max(
            (train_epoch - int(total_epochs / 2)), 0
        ) / ((total_epochs - epochs) - int(total_epochs / 2))
    else:
        T_metricL = max_tau_metricL
    return T_metricL


def adjust_learning_rate_cos(
    lr_ori, optimizer, epoch, total_epochs, args, warm_up_epochs=10
):
